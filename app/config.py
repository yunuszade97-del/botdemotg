...
class Settings(BaseSettings):
    ...
    # --- Склейка подряд идущих сообщений ------------------------------------
    # Сколько ждать продолжения, прежде чем отправить очередь в модель.
    # 0 — выключить (каждое сообщение обрабатывается отдельно).
    message_aggregation_delay: float = Field(default=1.5, ge=0)

    # --- Анти-спам ----------------------------------------------------------
    throttle_enabled: bool = True
    # 0 по умолчанию: очередь сообщений склеивается, а не отбрасывается —
    # иначе телефон, присланный вторым сообщением, просто терялся бы.
    # Ненулевое значение включает жёсткий отброс частых сообщений.
    throttle_min_interval: float = Field(default=0.0, ge=0)
    # Порог на реальный флуд. Стоимость сдерживается не им, а склейкой
    # сообщений и DAILY_LLM_CALLS_PER_USER.
    throttle_messages_per_minute: int = Field(default=30, ge=1)
